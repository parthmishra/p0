...
def flatten(ast):
	...
	if isinstance(ast,Module):
		...
		# if str(ast.name) in varList:
		# 	index=varList.index(str(ast.name))
		# 	ast.name=alias[index]
		# 	print "THIS IS THE NAME ITS GIVING", alias[index],index
		...
		# print "VAR LIST, ALIAS",varList,alias
	...
	if isinstance(ast,Module):
		...
		if isinstance(ast.expr,Add):
			...
			if isinstance(ast.expr.left,Const):
				...
				tempExprName=', -'+tempExprName+'(%ebp)'
				# print "TEMPEXPRNAMEEEE", tempExprName, tempExprLeft
				...
				# temp1=[temp1]
				if isinstance(ast.expr.right,Name):
					...
					# tempExprName=str(tempExprName)
					# tempExprName=', -'+tempExprName+'(%ebp)'
					print "ASDJHASKDJH=",tempExprRight,tempExprName

					# temp1='addl '+tempExprRight+tempExprName
					# temp1='addl '+'%eax'+tempExprName
					temp1='addl '+tempExprRight+', %eax'
					temp2='movl '+'%eax'+tempExprName
					return temp+[temp1]+[temp2]



				elif isinstance(ast.expr.right,Const):
					temp.append('movl '+'%eax'+tempExprName)
					# print 'in Assign, Add:',tempExprLeft,tempExprName
					tempExprRight=x86(ast.expr.right)
					print "TEMPEXPRRIGHT=",tempExprRight,tempExprName
					temp1='addl '+tempExprRight+tempExprName
				...
				tempExprName=str(tempExprName)
				tempExprName=', -'+tempExprName+'(%ebp)'
				print "LEFTLEFTLEFT", tempExprName,tempExprLeft
				...
				if isinstance(ast.expr.right,Name):
					...
					tempExprName=int(tempExprName[1])*4
					tempExprName=str(tempExprName)
					# print "IN Assign, Add, Name,Right", tempExprName,tempExprLeft
					...
					# print 'in Assign, Add:',tempExprLeft,tempExprName
					tempExprRight=x86(ast.expr.right)
					# print "TEMPEXPRRIGHT=",tempExprRight
					temp1='addl '+tempExprRight+tempExprName
			temp1=[temp1]
			...
			tempExprName=tempExprName.split('!!!',1)
			tempExprName=int(tempExprName[1])*4-4
			# tempExprName=int(tempExprName[-1:])
...
s
			...
			print "tempExpr=",tempExpr,tempExpr[0]
			if tempExpr[0]=='$':
				print "Went in one!", ['movl '+tempExpr+tempExprName]
				return ['movl '+tempExpr+tempExprName]
			else:
				tempExpr=tempExpr.split('!!!',1)
				tempExpr=int(tempExpr[1])*4-4
				tempExpr=str(tempExpr)
				tempExpr='-'+tempExpr+'(%ebp)'
				temp=[]
				temp.append('movl '+ tempExpr+ ' %eax')
				temp.append('movl '+'%eax'+tempExprName)
				print "Went in two!", temp
				return temp
				# return ['movl '+tempExpr+tempExprName]
	...
	if isinstance(ast,Add):
		...
		if isinstance(ast.right,Const):
			...
			tempExprRight=str(x86(ast.right))
			tempExprRight=tempExprRight.split('!!!',1)
			tempExprRight=int(tempExprRight[1])*4
			tempExprRight=str(tempExprRight)
			tempExprRight='-'+tempExprRight+'(%ebp)'
